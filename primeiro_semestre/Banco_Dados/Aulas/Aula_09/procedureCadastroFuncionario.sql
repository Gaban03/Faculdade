CREATE DEFINER=`root`@`localhost` PROCEDURE `cadastrofuncionario`(
	nome_colab VARCHAR(40), data_nascimento_colab DATE, data_admissao_colab DATE,
    pais_colab VARCHAR(40), estado_colab VARCHAR(2), cidade_colab VARCHAR(40),
	rua_colab VARCHAR(40), numero_colab INT, cep_colab VARCHAR(20), 
    telefone_fixo_colab VARCHAR(45), telefone_celular_colab VARCHAR(45),
    cargo_colab VARCHAR(30), departamento_colab VARCHAR(45), salario_colab FLOAT,
    nacionalidade_colab VARCHAR(20), saida VARCHAR(80), saida_rotulo VARCHAR(80)
)
BEGIN
    ...
    ELSE
		 INSERT INTO colaboradores(nome,data_nascimento,data_admissao,pais,
			...
			ELSE
                ...
                SET saida = "Usúario cadastrado com sucesso!";
			END IF;
		END IF;
